...
class ImageStackingPipeline:
    def __init__(self, root):
        ...
        # xyxymatch Frame
        self.xy_frame = ttk.LabelFrame(root, text="XYXY Match", padding=15)
        self.xy_frame.pack(fill="x", padx=30, pady=10)
        ...
        ttk.Separator(root, orient="horizontal").pack(pady=10)

        # Geomap Frame
        self.geomap_frame = ttk.LabelFrame(root, text="Geomap", padding=15)
        self.geomap_frame.pack(fill="x", padx=30, pady=10)
        ...
        # Geotran Frame
        self.geotran_frame = ttk.LabelFrame(root, text="Geotran", padding=15)
        self.geotran_frame.pack(fill="x", padx=30, pady=10)
        ...
        # ImCombine Frame
        self.imcombine_frame = ttk.LabelFrame(root, text="IMCOMBINE", padding=15)
        self.imcombine_frame.pack(fill="x", padx=30, pady=10)

        self._add_entry_frame(self.imcombine_frame, "Aligned Files", "Browse", self.browse_combine_input)
    ...
    def browse_combine_input(self, entry):
        ...
        if file:
            self.aligned_files_entry.delete(0, tk.END)
            self.aligned_files_entry.insert(0, file)
    ...
    def run_imcombine(self):
        try:
            inp = self.aligned_files_entry.get()
            if not inp:
                messagebox.showerror("Error", "Please provide input match file for IMCOMBINE.")
                return
            inp_list = re.findall(r"(/[^',\s]+\.fits)", inp)
            if not inp_list:
                messagebox.showerror("Error", "No valid FITS files found in input.")
                return
            self.output_dir = os.path.dirname(inp_list[0])
            os.chdir(self.output_dir)
            inp_str = ",".join(inp_list)
            out = self.get_next_filename(self.output_dir, "Stacked", ".fits")
            iraf.unlearn(iraf.imcombine)
            iraf.imcombine(input=inp_str, output=out, combine="lmedian", reject="ccdclip")
            messagebox.showinfo("Success", f"IMCOMBINE completed.\nOutput: {out}")
